    ...
    for chr_num in chromosomes:
        ...
        for idx, loop in loops.iterrows():
            if not is_mask_corner_peak:
                ...
                mask[int(loop.x - mask_window):int(loop.x + mask_window), int(loop.y - mask_window):int(loop.y + mask_window)] = 1
                mask[int(loop.y - mask_window):int(loop.y + mask_window), int(loop.x - mask_window):int(loop.x + mask_window)] = 1
